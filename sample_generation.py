...
def simulate_counterfactual_D(df_arm, counter_clf, beta_d, beta_y, beta_u, use_latent):
    if use_latent:  # simulate D0 with X and U
        ...
        beta = np.concatenate(([counter_clf.intercept_[0]],
                               ...
                               [beta_u]))
    else:  # simulate D0 with X, D1, Y1*D1 and vise versa
        att = np.hstack([np.ones(shape=(df_arm.shape[0], 1)),
                         np.vstack(df_arm.x),
...
def simulate_counterfactual_Y(df_arm, counter_rl, beta_d, beta_y, beta_u, col, use_latent):
    att = np.hstack([np.ones(shape=(df_arm.shape[0], 1)),
                     np.vstack(df_arm.x),
                     df_arm[col].fillna(0).to_numpy()])
    if use_latent:  # simulate Y0 with X, D_obs and U
        beta = np.concatenate(([counter_rl.intercept_],
                               counter_rl.coef_.ravel(),
                               [beta_u]))
    else:  # simulate Y0 with X, D0, D1, Y1*D1 and vise versa
        beta = np.concatenate(([counter_rl.intercept_],
                               counter_rl.coef_.ravel(),
                               [beta_d, beta_y]))
    logits = np.dot(att, beta).astype(float)
...
def simulate_counterfactuals(df, beta_d=1.0, beta_y=1.0, beta_u=1.0, use_latent=True,
    ...
    treatment_df['u'] = treatment_df.index.to_series().apply(lambda idx: np.random.RandomState(idx).normal())
    control_df['u'] = control_df.index.to_series().apply(lambda idx: np.random.RandomState(idx).normal())
    ...
    if use_latent:
        rl_att = np.hstack((np.vstack(control_df.x), control_df.D_obs.values.reshape(-1, 1)))
        control_lr = LinearRegression().fit(rl_att, control_df.Y_obs.fillna(0))
        rl_att = np.hstack((np.vstack(treatment_df.x), treatment_df.D_obs.values.reshape(-1, 1)))
        treatment_lr = LinearRegression().fit(rl_att, treatment_df.Y_obs.fillna(0))
    else:
