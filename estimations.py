...
def estimate_beta_d_from_realizations(true_beta_d_for_comparison: Optional[List[float]],
                                      t: np.array, x: np.array, d_obs: np.array) -> List[float]:
    random.seed(default_random_seed)

    features = [[t_i, *np.ravel(x_i)] for t_i, x_i in zip(t, x)]
    y = d_obs
